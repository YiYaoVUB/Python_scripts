...
str_obsfile = 'D:\Forcing_tower\Philippines\\Philippines.csv'
pd_reader = pd.read_csv(str_obsfile, encoding= 'unicode_escape')
data_forc = np.array(pd_reader)
lh_obs = data_forc[:, 27] #C
sh_obs = data_forc[:, 26] #%
#gpp_obs = data_forc[:, 20]
#sf_obs = data_forc[:, 46] * 100
sm_obs = data_forc[:, 18] / 100
lw_obs = data_forc[:, 32]
nee_obs = data_forc[:, 25]  * 12 / 1000000
sw_obs = data_forc[:, 30]

str_simfile1 = 'D:\Tower_result\\'
site_name = 'Philippines'

bias_noirr = np.zeros([6,1])
bias_drip = np.zeros([6,1])
bias_flood = np.zeros([6,1])
rmse_noirr = np.zeros([6,1])
rmse_drip = np.zeros([6,1])
rmse_flood = np.zeros([6,1])

index = 0
for str_var in ['EFLX_LH_TOT', 'FSH', 'NEE', 'SWup', 'LWup', 'H2OSOI']:
    ...
    str_file_flood = 'D:\Tower_result\\' + site_name + '_pool\\' + site_name + '_flood_' + str_var
    ...
    bias_noirr[index] = bias_noirr_sum[0]/num
    bias_drip[index] = bias_drip_sum[0]/num
    bias_flood[index] = bias_flood_sum[0]/num
    rmse_noirr[index] = math.sqrt(rmse_noirr_sum[0]/num)
    rmse_drip[index] = math.sqrt(rmse_drip_sum[0]/num)
    rmse_flood[index] = math.sqrt(rmse_flood_sum[0]/num)
...
np.savetxt("E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\\Philippines_Bias.csv", bias, delimiter=',')
np.savetxt("E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\\Philippines_Rmse.csv", rmse, delimiter=',')
