...
str_infile = 'D:\Forcing_tower\Philippines\\Philippines.csv'
pd_reader = pd.read_csv(str_infile, encoding= 'unicode_escape')
data_forc = np.array(pd_reader)
temp = data_forc[:, 38] #C
humi = data_forc[:, 36] #%
wids = data_forc[:, 41] #m/s
pres = data_forc[:, 37] * 1000
sola = data_forc[:, 29]
prec = data_forc[:, 39] / 86400
long = data_forc[:, 31]
...
for year in range(2013, 2014):
    ...
    for month in range(1, 13):
        str_month = str(month)
        if month < 10:
            strfile = 'D:\Forcing_tower\\Philippines\\' + str_year + '-0' + str_month + '.nc'
        else:
            strfile = 'D:\Forcing_tower\\Philippines\\' + str_year + '-' + str_month + '.nc'
        ...
        edgee[0] = 121.5625
        longxy[0, 0] = 121.2500
        edgen[0] = 14.569190600522186
        edges[0] = 14.099216710182759
        latixy[0, 0] = 14.334203655352471
