...
str_infile = 'D:\Forcing_tower\Humnoke\\Humnoke_C.csv'
pd_reader = pd.read_csv(str_infile, encoding= 'unicode_escape')
data_forc = np.array(pd_reader)
temp = data_forc[:, 39] #C
humi = data_forc[:, 37] #%
wids = data_forc[:, 44] #m/s
pres = data_forc[:, 38] * 1000
sola = data_forc[:, 29]
prec = data_forc[:, 40] / 86400
long = data_forc[:, 31]

for year in range(2017, 2018):
    str_year = str(year)
    if (year % 4) == 0:
        day_start = [0, 31, 60, 91, 121, 152, 182, 213]
        day_end = [31, 60, 91, 121, 152, 182, 213, 239]
        gap_add = 366
    else:
        day_start = [0, 31, 59, 90, 120, 151, 181, 212]
        day_end = [31, 59, 90, 120, 151, 181, 212, 238]
        gap_add = 365

    for month in range(1, 9):
        str_month = str(month)
        if month < 10:
            strfile = 'D:\Forcing_tower\\Humnoke\\' + str_year + '-0' + str_month + '.nc'
        else:
            strfile = 'D:\Forcing_tower\\Humnoke\\' + str_year + '-' + str_month + '.nc'
        ...
        edgee[0] = 268.4375
        longxy[0, 0] = 268.1250
        edgen[0] = 34.778067885117450
        edges[0] = 34.308093994778034
        latixy[0, 0] = 34.543080939947740
