...
csvframe_noi = pd.read_csv("D:\Tower_result\Global_simulations\\NOI.csv",header=None)
csvframe_ctl = pd.read_csv("D:\Tower_result\Global_simulations\\CTL.csv",header=None)
csvframe_irr = pd.read_csv("D:\Tower_result\Global_simulations\\IRR.csv",header=None)
...
for i in range(len(x)):
    x[i] = x[i] + width
plt.bar(x, plot_T_ctl_noi, width=width, label='T',fc = 'dodgerblue')
for i in range(len(x)):
    x[i] = x[i] + width
...
plt.legend(loc='best', fontsize=20)
ax1.set_title('CTL - NOI',loc='right',fontsize = 22)
plt.ylabel(r"Changes (mm/year) / ($kg^2/m^3$)", fontsize=20)
...
for i in range(len(x)):
    x[i] = x[i] + width
plt.bar(x, plot_T_irr_noi, width=width, label='T',fc = 'dodgerblue')
for i in range(len(x)):
    x[i] = x[i] + width
...
for i in range(len(x)):
    x[i] = x[i] + width
plt.bar(x, plot_T_irr_ctl, width=width, label='T',fc = 'dodgerblue')
for i in range(len(x)):
    x[i] = x[i] + width
...
plt.savefig('E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\\200.png')
#plt.show()
print('test')