...
str_obsfile = 'D:\Forcing_tower\Cheorwon\\Cheorwon.csv'
pd_reader = pd.read_csv(str_obsfile, encoding= 'unicode_escape')
data_forc = np.array(pd_reader)
lh_obs = data_forc[:, 30] #C
sh_obs = data_forc[:, 29] #%
#gpp_obs = data_forc[:, 20]
sf_obs = data_forc[:, 43] * 100
lw_obs = data_forc[:, 35]
nee_obs = data_forc[:, 28] * 12 / 1000000
sw_obs = data_forc[:, 33]
...
np.savetxt("E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\Cheorwon_Bias.csv", bias, delimiter=',')
np.savetxt("E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\Cheorwon_Rmse.csv", rmse, delimiter=',')
