...
str_obsfile = 'D:\Forcing_tower\Castellaro\\Castellaro.csv'
pd_reader = pd.read_csv(str_obsfile, encoding= 'unicode_escape')
data_forc = np.array(pd_reader)
lh_obs = data_forc[:, 28] #C
sh_obs = data_forc[:, 27] #%
#gpp_obs = data_forc[:, 20]
sm_obs = data_forc[:, 19]/100
#lw_obs = data_forc[:, 30]
nee_obs = data_forc[:, 26] * 12 / 1000000
sw_obs = data_forc[:, 31]

str_simfile1 = 'D:\Tower_result\\'
site_name = 'Castellaro'
...
index = 0
for str_var in ['EFLX_LH_TOT', 'FSH', 'H2OSOI', 'NEE', 'SWup']:
    ...
    str_file_flood = 'D:\Tower_result\\' + site_name + '_flood\\' + site_name + '_flood_' + str_var
...
np.savetxt("E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\Castellaro_Bias.csv", bias, delimiter=',')
np.savetxt("E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\Castellaro_Rmse.csv", rmse, delimiter=',')
print('test')
